    ...
    while (this.queue.length > 0) {
      ...
      if (uploadFn) {
        try {
          ...
          console.error('B2 upload queue error:', error);
        }
      }
    }
...
const b2UploadQueue = new B2UploadQueue();
  ...
  try {
    ...
    try {
      ...
      if (b2Bucket && b2Bucket !== 'robotlib' && b2Bucket.trim() !== '') {
        ...
        console.log(`Adding to B2 upload queue: ${localPath} -> ${b2Bucket}/${remoteFileName}`);

        // 将上传任务添加到队列，确保同一时间只有一个文件在上传
        await b2UploadQueue.add(async () => {
          console.log(`[Queue] Starting B2 upload: ${remoteFileName}`);
          ...
          console.log('[Queue] B2 upload stdout:', stdout);
          if (stderr) console.log('[Queue] B2 upload stderr:', stderr);
          
          console.log(`[Queue] B2 upload completed: ${remoteFileName}`);
        });
...
router.post('/upload', authenticateToken, requireAdmin, upload.single('file'), async (req: AuthRequest, res: Response) => {
  try {
    ...
    // 2. 尝试同步到 Backblaze B2（使用队列确保不会并发上传）
    let b2Synced = false;
    let b2Path = '';
    let syncError: string | null = null;
