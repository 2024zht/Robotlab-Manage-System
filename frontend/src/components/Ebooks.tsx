...
const Ebooks: React.FC = () => {
  ...
  const retryUpload = async (taskId: string) => {
    ...
    if (task.uploadedFileId) {
      try {
        ...
        console.log('已删除之前失败的文件:', task.uploadedFileId);
      } catch (error) {
        console.error('删除失败文件出错:', error);
      }
    }
    ...
    // 如果当前没有在上传，立即开始上传
    if (!isUploading) {
      const waitingTasks = uploadTasks.map(t => 
        t.id === taskId ? { ...t, status: 'waiting' as const } : t
      );
      processUploadQueue(waitingTasks);
    }
  };
  ...
  const clearFailedTasks = async () => {
    ...
    if (!window.confirm(`确定要删除 ${failedTasks.length} 个失败/取消的任务吗？这将删除服务器上的临时文件。`)) {
      return;
    }
  ...
  const retryAllFailedTasks = async () => {
    ...
    if (!window.confirm(`确定要重新上传 ${failedTasks.length} 个失败/取消的任务吗？`)) {
      return;
    }
    ...
    // 统一开始上传所有等待的任务
    if (!isUploading) {
      processUploadQueue(uploadTasks);
    }
  };
        ...
        <div className="fixed bottom-6 right-6 z-50 w-[480px] bg-white rounded-lg shadow-2xl border border-gray-200 overflow-hidden">
          <div className="p-4 bg-gradient-to-r from-blue-600 to-blue-700 text-white flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              ...
              {uploadTasks.some(task => task.status === 'error' || task.status === 'cancelled') && (
                <>
                  <button
                    onClick={retryAllFailedTasks}
                    className="text-xs px-2 py-1 bg-green-500 bg-opacity-80 rounded hover:bg-opacity-100 transition flex items-center gap-1"
                    title="重新上传所有失败/取消的任务"
                  >
            ...
            {uploadTasks.map((task) => (
              <div key={task.id} className="p-4 border-b border-gray-100 hover:bg-gray-50">
                ...
                <div className="text-xs text-gray-600 mb-2">
                  ...
                  {task.status === 'error' && (
                    <div className="space-y-2">
                      <span className="text-red-600 block">{task.error}</span>
                      <button
                        onClick={() => retryUpload(task.id)}
                        className="inline-flex items-center px-3 py-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition text-xs"
                      >
                  ...
                  {task.status === 'cancelled' && (
                    <div className="space-y-2">
                      <span className="text-gray-600 block">{task.error || '已取消'}</span>
                      <button
                        onClick={() => retryUpload(task.id)}
                        className="inline-flex items-center px-3 py-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition text-xs"
                      >
